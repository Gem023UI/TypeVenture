...
const FontPairingGame = () => {
  ...
  const handleOptionClick = (pairName) => {
  ...
  const handleSubmitAnswer = () => {
    ...
    const correct = selectedPair === currentQuestion.correctPairName;
  ...
  if (loading) {
    return (
      <div className="fontpair-game-page">
  ...
  return (
    <div className="fontpair-game-page">
      <div className="fontpair-game-container">
        {/* Progress Section */}
        ...
        <div className="pairing-container">
          {/* Left Side - Given Font */}
          <div className="given-font-section">
            <h2 className="given-font-title">
              Find the perfect pairing for this font:
            </h2>
            <div className="given-font-display">
              <img 
                src={currentQuestion.givenFontImage} 
                alt="Given font"
                className="given-font-image"
              />
            </div>
            <div className="given-font-name">
              {currentQuestion.givenFontName}
            </div>
          </div>

          {/* Right Side - Options */}
          <div className="options-section">
            <h3 className="options-title">Select the best matching font:</h3>
            
            {isAnswered && (
              <div className={`feedback-message ${isCorrect ? 'correct' : 'incorrect'}`}>
                {isCorrect ? (
                  <>✅ Perfect pairing! {currentQuestion.correctPairName} works beautifully with {currentQuestion.givenFontName}</>
                ) : (
                  <>❌ Not quite. The best pairing is {currentQuestion.correctPairName}</>
                )}
              </div>
            )}
            ...
            <div className="options-grid">
              {currentQuestion.options.map((option, index) => {
                ...
                if (isAnswered) {
                  cardClass += ' disabled';
                  if (option.pairName === currentQuestion.correctPairName) {
                    cardClass += ' correct';
                  } else if (option.pairName === selectedPair && !isCorrect) {
                    cardClass += ' incorrect';
                  }
                } else if (option.pairName === selectedPair) {
                  cardClass += ' selected';
                }
                ...
                return (
                  <div
                    key={index}
                    className={cardClass}
                    onClick={() => handleOptionClick(option.pairName)}
                  >
                    <div className="option-image-container">
                      <img 
                        src={option.pairImage} 
                        alt={option.pairName}
                        className="option-font-image"
                      />
                    </div>
                    <div className="option-font-name">
                      {option.pairName}
                    </div>
                  </div>
                );
...
export default FontPairingGame;